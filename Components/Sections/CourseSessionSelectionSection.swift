...
/// 课节选择区（第1节～第12节）
/// 4列网格，水平间距1，垂直间距2，固定宽191
struct CourseSessionSelectionSection: View {
    let totalSessions: Int              // 总节数，通常为12
    @Binding var selected: Set<Int>     // 已选节次（1-based）
        ...
        .gesture(
            DragGesture(minimumDistance: 0, coordinateSpace: .local)
                .onChanged { v in
                    ...
                    for i in lo...hi {
                        let n = i + 1
                        if n >= 1 && n <= totalSessions { if adding { next.insert(n) } else { next.remove(n) } }
                    }
                    selected = next
                }
                .onEnded { v in
    ...
    private var rowCount: Int { (totalSessions + cols - 1) / cols }
    private func cellsInRow(_ r: Int) -> Int { min(cols, totalSessions - r * cols) }

    private func cellIndex(at p: CGPoint) -> Int? {
        let col = Int(p.x / (cellW + hGap))
        let row = Int(p.y / (cellH + vGap))
        let idx = row * cols + col
        guard col >= 0, col < cols, idx >= 0, idx < totalSessions else { return nil }
        return idx
    }
}

#Preview {
    @Previewable @State var selected: Set<Int> = [1, 2, 3, 4]
    CourseSessionSelectionSection(totalSessions: 12, selected: $selected)
        .padding(24)
